...
class MCUpdater 
{
    ...
    {  
		Console.WriteLine("[MCSU]: Server Updater Started");
		
		ServicePointManager.SecurityProtocol |= SecurityProtocolType.Tls11 | SecurityProtocolType.Tls12;

        Console.WriteLine("[MCSU]: Stopping Server...");
        SetServer(false);
		...
		using (WebClient client = new WebClient())
		{
			...
			Console.WriteLine("[MCSU]: Fetching most recent version...");
            string result = client.DownloadString("https://minecraft.net/en-us/download/server/bedrock/");

			Regex rx = new Regex(@"(https:\/\/minecraft\.azureedge\.net\/bin-linux\/bedrock-server-)(.*)(\.zip)");
			Match match = rx.Match(result);
			string link = match.Value;
			string version = match.Groups[2].Value;
			Console.WriteLine("[MCSU]: Version Found -> " + version);
			...
			try {
				...
				Console.WriteLine("[MCSU]: Current Version -> " + oldversion);
				if (version == oldversion)
				{
					Console.WriteLine("[MCSU]: Server is up to date! Now exiting...");
					Thread.Sleep(2000);
					return;
				}
			} catch {
				Console.WriteLine("[MCSU]: Current Version not found. Updating anyway...");
			}
			File.WriteAllText("version.txt", version);
			
			Console.WriteLine("[MCSU]: Downloading new server...");
			client.DownloadFile(link, "server.temp");
			Console.WriteLine("[MCSU]: Finished Downloading");
		}
		
		Console.WriteLine("[MCSU]: Extracting new server...");
		ZipStorer zip = ZipStorer.Open("server.temp", FileAccess.Read);
		List<ZipStorer.ZipFileEntry> dir = zip.ReadCentralDir();
		foreach (ZipStorer.ZipFileEntry entry in dir)
		{
			if (entry.FilenameInZip != "server.properties" && entry.FilenameInZip != "whitelist.json" && entry.FilenameInZip != "permissions.json")
			{
				zip.ExtractFile(entry, Path.Combine(Directory.GetCurrentDirectory(), entry.FilenameInZip));
			}
		}
		zip.Close();
		Console.WriteLine("[MCSU]: Finished Extracting");
		
		Console.WriteLine("[MCSU]: Cleaning up...");
		File.Delete("server.temp");
        Console.WriteLine("[MCSU]: Starting Server...");
        SetServer(true);
		Console.WriteLine("[MCSU]: Finished! Now exiting...");
			
		Thread.Sleep(2000);
	}

    public static void SetServer(bool on) {
        Process proc = new System.Diagnostics.Process();
        proc.StartInfo.FileName = "/bin/bash";
        proc.StartInfo.Arguments = "-c 'service mc-serve " + (on ? "start" : "stop") + "'";
...
//Thanks Jaime Olivares! Super useful!
